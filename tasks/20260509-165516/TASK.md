...
- [ ] `SCUFRIS_TELEMETRY=1` produces one JSONL line per sub-agent tool call.
- [ ] When unset, no file is created and no perf cost beyond an `if` check.
- [ ] All four sub-agents log correctly (knowledge, coding, journal,
      utilities).
- [ ] Refused calls (matching `cannot_handle:` prefix) are tagged
      `outcome: "refused"`.
- [ ] Errors during sub-agent execution log `outcome: "error"` and don't
      crash the parent turn.
- [ ] `turn_id` correctly groups multi-delegation turns (manually verified
      with a follow-up question that triggers two sub-agents).
- [ ] Log rotates at 10 MB without data loss.
